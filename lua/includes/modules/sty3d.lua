...
Copyright thelastpenguin™
	...
	Unauthorized use of this code either in part or in whole will result in a take down request.
...
hook.Add( "KeyPress", "pdoors.3d2d.KeyPress", function( ply, key )
	if key ~= IN_USE then return end
	isUsePressed = true
end)

hook.Add("KeyRelease", "pdoors.3d2d.KeyPress", function(ply, key)
	if key ~= IN_USE then return end
	isUsePressed = false
end)
...
function Panel:STYDraw3D(pos, ang, scale, noCursor)
	...
	cam.Start3D2D(pos, ang, scale)
		...
		surface.DrawRect(gui.MouseX(), gui.MouseY(), 10, 10)
	cam.End3D2D()
